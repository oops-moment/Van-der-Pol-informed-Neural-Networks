...
class PINN:
    def __init__(self, mu, training):
        ...
        self.gradient_tt = (self.gradient_t.diff()/self.t_diff).iloc[1:] # d2x/dt2
    ...
    def load_data(self, filename):
        ...
        self.training_set = self.data.iloc[:, 3]
    ...
    def convert(self, offset=30, in_val=35, out_val=10):
        ...
        cols_to_drop = [f"var{i}(t-{j})" for j in range(in_val, 1, -1) for i in range(2, 4)]
        ...
        self.forecastX = self.forecastX.reshape((self.forecastX.shape[0], 1, self.forecastX.shape[1]))
    ...
    def lorenz_loss_fn(y_true, y_pred):
        mu = tf.Variable(4, name="mu", trainable=True, dtype=tf.float32)
        splitr = 0.8
        sigma = 10
        rho = 28
    ...
    def train_model(self, type):
        ...
        self.model.add(LSTM(50, input_shape=(self.trainX.shape[1], self.trainX.shape[2])))
        ...
        self.history = self.model.fit(self.trainX[:int(splitr*self.trainX.shape[0])], self.trainy[:int(splitr*self.trainX.shape[0])], epochs=100, batch_size=64, validation_data=(self.trainX[int(splitr*self.trainX.shape[0]):self.trainX.shape[0]], self.trainy[int(splitr*self.trainX.shape[0]):self.trainX.shape[0]]), shuffle=False)

    def evaluate_model(self):
        self.forecast_without_mc = self.forecastX
        yhat_without_mc = self.model.predict(self.forecast_without_mc) # Step Ahead Prediction
        self.forecast_without_mc = self.forecast_without_mc.reshape((self.forecast_without_mc.shape[0], self.forecast_without_mc.shape[2])) # Historical Input
